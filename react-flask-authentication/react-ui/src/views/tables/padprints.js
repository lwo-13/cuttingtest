...
const CreatePadPrintModal = ({ open, handleClose, onCreated }) => {
    ...
    const handleSubmit = async () => {
      try {
        const response = await axios.post("http://127.0.0.1:5000/api/padprint/", formData);
...
const EditPadPrintModal = ({ open, handleClose, padPrint, onUpdated }) => {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.put(`http://127.0.0.1:5000/api/padprint/${padPrint.id}`, formData);
...
const PadPrints = () => {
    ...
    try {
      ...
      for (const id of selectedItems) {
        const response = await axios.post(
          `http://127.0.0.1:5000/api/padprint/upload-image/${id}`,
          formData,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:5000/api/padprint/${id}`);
