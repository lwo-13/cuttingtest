...
const MattressTable = () => {
    ...
    const fetchMattresses = () => {
        setLoading(true);
        axios.get('http://172.27.57.210:500/api/mattress/all_with_details')
            .then((response) => {
            ...
            try {
                ...
                try {
                    const response = await axios.get(
                        `http://172.27.57.210:500/api/markers/marker_pcs?marker_name=${markerName}`
                    );
                    if (response.data.success) {
                ...
                const orderLinesResponse = await axios.get(
                    `http://172.27.57.210:500/api/orders/order_lines?order_commessa=${encodeURIComponent(orderCommessa)}`
                );
                ...
                const padPrintResponse = await axios.get(
                    `http://172.27.57.210:500/api/padprint/filter?season=${encodeURIComponent(season)}&style=${encodeURIComponent(style)}&color=${encodeURIComponent(color)}`
                );
                ...
                await axios.put(`http://172.27.57.210:500/api/mattress/update_print_travel`, {
    ...
    const handlePrintBG = async () => {
        ...
        for (const mattress of selectedMattresses) {
            try {
                ...
                let markerLines = [];
                try {
                    const response = await axios.get(`http://172.27.57.210:500/api/markers/marker_pcs?marker_name=${markerName}`);
                    if (response.data.success) {
                        markerLines = response.data.marker_lines; 
                    }
                } catch (error) {
